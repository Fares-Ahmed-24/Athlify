...
void showClubRatingDialog({
  ...
  showDialog(
    context: context,
    builder: (context) => ClubRatingDialog(
      userId: userId,
      itemId: itemId,
      itemType: itemType,
    ),
...
class ClubRatingDialog extends StatefulWidget {
  final String userId;
  final String itemId;
  final String itemType;

  const ClubRatingDialog({
    super.key,
  ...
  @override
  State<ClubRatingDialog> createState() => _ClubRatingDialogState();
}

class _ClubRatingDialogState extends State<ClubRatingDialog> {
  double _rating = 0;
  ...
  void _submitRating() async {
    ...
    if (success) {
      ...
      /// ❌ شيل `Navigator.pop(context)` من هنا

      // ✅ اعرض الـ AwesomeDialog وبعد ما المستخدم يضغط OK، اقفل الـ dialog الأصلي
      AwesomeDialog(
        context: context,
        ...
        btnOkOnPress: () {
          Navigator.pop(context); // close ClubRatingDialog safely
        },
      ).show();
    } else {
      AwesomeDialog(
